    ...
    <div className="relative max-w-4xl mx-auto my-10 bg-gradient-to-br from-indigo-900 via-slate-900 to-cyan-900 text-white rounded-2xl shadow-2xl p-8">
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start mb-8">
        <div className="flex flex-col items-center md:items-start">
          <img
            src={tv.poster_path ? `https://image.tmdb.org/t/p/w342/${tv.poster_path}` : '/No-Poster.png'}
            alt={tv.name}
          ...
          <div className="flex items-center justify-center gap-3 mt-4 w-full">
            ...
            <span className="text-slate-300 text-lg">
              • {tv.number_of_seasons ? `${tv.number_of_seasons} Season${tv.number_of_seasons > 1 ? 's' : ''}` : 'N/A'}
            </span>
          </div>
        </div>
        <div className="flex-1 flex flex-col items-center">
          <h1 className="text-4xl font-bold mb-4 leading-none text-cyan-300 text-center">{tv.name}</h1>
          ...
          <div className="w-full">
            <p className="text-slate-300 mb-2 text-left">
              <strong>Rating:</strong> {tv.vote_average ? tv.vote_average.toFixed(1) : 'N/A'}
            </p>
            <p className="text-slate-300 mb-2 text-left">
              <strong>Language:</strong> {tv.original_language?.toUpperCase()}
            </p>
            <p className="text-slate-300 mb-2 text-left">
              <strong>Release Date:</strong> {tv.first_air_date ? tv.first_air_date : 'N/A'}
            </p>
          </div>
          <h3 className="text-xl font-semibold text-cyan-300 mt-3 mb-2 w-full text-left">Overview</h3>
          <p className="leading-relaxed text-slate-200 mb-2">{tv.overview}</p>
          {trailer && (
            <button
...
export const TvShowDetailLoader = async ({ params }) => {
  const { id } = params;
  // Fetch TV show details
  const tvRes = await fetch(`${API_BASE_URL}/tv/${id}`, API_OPTIONS);
  if (!tvRes.ok) throw new Error('Failed to fetch TV show details');
  const tv = await tvRes.json();

  // Fetch credits (cast)
  const creditsRes = await fetch(`${API_BASE_URL}/tv/${id}/credits`, API_OPTIONS);
  if (!creditsRes.ok) throw new Error('Failed to fetch TV show credits');
  const credits = await creditsRes.json();

  // Fetch videos (trailers)
  const videosRes = await fetch(`${API_BASE_URL}/tv/${id}/videos`, API_OPTIONS);
  if (!videosRes.ok) throw new Error('Failed to fetch TV show videos');
  const videos = await videosRes.json();

  return { tv, credits, videos };
};